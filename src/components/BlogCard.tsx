...
export default function BlogCard({ blog }: BlogCardProps) {
  const { _id, title, description, categoryId, imageUrl, readingTime, createdAt } = blog;
  const categoryName = typeof categoryId === 'object' ? categoryId?.name : 'General';

  const formatDate = (dateString: string) => {
    const date = new Date(dateString);
    return isNaN(date.getTime()) ? "Reciente" : date.toLocaleDateString('es-ES', { day: 'numeric', month: 'short' });
  };
  ...
  return (
    <div className="group bg-white rounded-[40px] overflow-hidden border border-slate-100 shadow-sm hover:shadow-2xl transition-all duration-500 flex flex-col h-full">
      {/* Contenedor de Imagen con Padding para estilo Galería */}
      <div className="relative p-3">
        <div className="relative h-64 w-full overflow-hidden rounded-[32px] bg-slate-100">
          {imageUrl ? (
            ...
            <div className="w-full h-full flex items-center justify-center bg-slate-200 text-slate-400 font-black text-[10px] uppercase tracking-widest">Sin Portada</div>
          )}
      ...
      <div className="px-8 pb-8 pt-2 flex flex-col flex-grow">
        ...
        <div className="mt-auto">
          <Link href={`/blog/${_id}`} className="inline-flex items-center gap-2 text-blue-600 font-black text-xs uppercase tracking-widest group/btn">
            Leer enseñanza <span className="transition-transform group-hover/btn:translate-x-1">→</span>
          </Link>
        </div>
      </div>
    </div>
