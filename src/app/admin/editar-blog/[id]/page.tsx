...
// Importación dinámica para evitar errores de hidratación y SSR
...
export default function EditBlogPage() {
    ...
    try {
      const finalImagesArray = await Promise.all(
        images.map(async (img) => {
          ...
          return img.src; // Mantiene la URL si ya existía en la DB
        })
      );
      ...
      const res = await fetch(`/api/blogs/${id}`, {
        ...
        body: JSON.stringify({
          ...formData,
          imageUrl: finalImagesArray,
          content: content // Guardamos el HTML del editor
        }),
      });
      ...
      alert('Error al actualizar la enseñanza');
    } finally {
      setUpdating(false);
    }
  };
  ...
  if (loading) return (
    <div className="min-h-screen flex items-center justify-center bg-white">
      <div className="text-center animate-pulse text-slate-400 font-black uppercase tracking-widest text-xs">Cargando datos...</div>
    </div>
  );

  return (
    <div className="max-w-4xl mx-auto py-10 px-4">
      <button onClick={() => router.back()} className="mb-8 flex items-center gap-2 text-slate-400 font-black text-xs uppercase tracking-widest group hover:text-blue-600 transition-colors">
      ...
      <form onSubmit={handleSubmit} className="p-8 space-y-8 bg-white rounded-[40px] shadow-sm border border-slate-100">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="space-y-2">
            ...
            <input name="title" value={formData.title} onChange={handleChange} required className="w-full px-6 py-4 rounded-2xl bg-slate-50 border-none outline-none focus:ring-2 focus:ring-blue-500 shadow-inner" />
          </div>
          <div className="space-y-2">
            <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-2">Doctrina</label>
            <select name="categoryId" value={formData.categoryId} onChange={handleChange} className="w-full px-6 py-4 rounded-2xl bg-slate-50 border-none outline-none focus:ring-2 focus:ring-blue-500 appearance-none shadow-inner cursor-pointer">
        ...
        <div className="space-y-2">
          ...
          <textarea name="description" value={formData.description} onChange={handleChange} required rows={2} className="w-full px-6 py-4 rounded-2xl bg-slate-50 border-none outline-none resize-none focus:ring-2 focus:ring-blue-500 shadow-inner" />
        </div>
        ...
        <div className="space-y-2">
          ...
          <div className="bg-white rounded-[32px] overflow-hidden border border-slate-200 shadow-inner">
            <ReactQuill 
              theme="snow" 
              value={content} 
              onChange={setContent} 
        ...
        <button 
          ...
          className={`w-full py-5 rounded-[24px] font-black text-sm uppercase tracking-[0.2em] transition-all ${
            updating ? 'bg-slate-100 text-slate-400 cursor-not-allowed' : 'bg-blue-600 text-white hover:bg-blue-700 shadow-xl shadow-blue-200 active:scale-[0.98]'
          }`}
        >
